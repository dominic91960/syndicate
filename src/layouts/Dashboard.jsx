...
const DashboardLayout = () => {
  ...
  return (
    <>
      ...
      {/* Main */}
      <main className="dashboard font-montserrat relative h-[calc(100dvh-82px)] overflow-hidden bg-[#0A0A0A] bg-fixed text-[12px] text-white sm:text-[13px] md:text-[14px] lg:text-[14.5px] xl:text-[15px] 2xl:text-[16px]">
        <div className="section-padding container mx-auto grid h-full gap-[2em] lg:grid-cols-4 2xl:grid-cols-5">
          <section
            className={`absolute left-0 h-full border-x border-white/30 bg-black/50 pt-[2em] backdrop-blur-sm lg:static lg:col-span-1 lg:animate-none ${isSidebarOpen ? "animate-sidebar-open" : "animate-sidebar-close"}`}
          >
            <Sidebar onNavClick={handleSidebarClose} />
          </section>
          <section className="h-full px-[1em] pt-[2em] lg:col-span-3 2xl:col-span-4">
            <Outlet />
          </section>
        </div>
      </main>
